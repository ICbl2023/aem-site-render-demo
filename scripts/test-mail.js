...
if(!configResponse.ok || !(await configResponse.json()).enabled)throw new Error("Le serveur n’annonce pas un envoi configuré. Aucun email de test envoyé.");
...
for(const workflow of ["ants","permis"]){
 ...
 // Mois du justificatif de domicile : obligatoire pour une facture, et daté du mois courant pour rester valide au fil du temps.
 ...
 if(!response.ok||!result?.ok||result.submissionId!==id)throw new Error("Test "+workflow+" non confirmé (HTTP "+response.status+", référence "+id+"). Vérifiez côté administration avant de relancer.");
 console.log(workflow.toUpperCase()+" : accepté par le relais SMTP. Référence : "+id);
}
console.log("Vérifiez la boîte destinataire configurée : deux notifications TEST-AEM. Les pièces sont dans l’espace admin (elles ne sont pas jointes au mail, AEM_MAIL_ATTACHMENTS=0). L’acceptation SMTP ne prouve pas la réception dans Gmail.");
